...
from syk_core.entegrasyon.syk_goruntu_kanit_manifesti import (
    SYKGoruntuKanitManifesti,
)
...
def test_goruntu_kanit_manifesti_olusturulur(tmp_path):
    motor = SYKGoruntuKanitManifesti(_kok())
    cikti = tmp_path / "manifest.json"

    belge = motor.kaydet(cikti)

    assert belge["kaynak_sayisi"] == 3
    assert belge["test_dosyasi_sayisi"] == 3
...
def test_kaynak_dosyasi_degistirilirse_reddedilir(tmp_path):
    ...
    for goreli in (
        SYKGoruntuKanitManifesti.KAYNAKLAR
        + SYKGoruntuKanitManifesti.TESTLER
    ):
    ...
    motor = SYKGoruntuKanitManifesti(proje)
    cikti = tmp_path / "manifest.json"
    motor.kaydet(cikti)
...
def test_eksik_dosya_manifest_olusturmayi_durdurur(tmp_path):
    motor = SYKGoruntuKanitManifesti(tmp_path)
