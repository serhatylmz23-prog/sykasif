...
from syk_core.entegrasyon.syk_muhur_teslim_oncesi_manifesti import (
    SYKMuhurTeslimOncesiManifesti,
)
...
def test_muhur_teslim_oncesi_manifesti_olusturulur(tmp_path):
    motor = SYKMuhurTeslimOncesiManifesti(_kok())
...
def test_kaynak_degistirilirse_reddedilir(tmp_path):
    ...
    for goreli in (
        SYKMuhurTeslimOncesiManifesti.KAYNAKLAR
        + SYKMuhurTeslimOncesiManifesti.TESTLER
    ):
    ...
    motor = SYKMuhurTeslimOncesiManifesti(proje)
    cikti = tmp_path / "manifest.json"
    motor.kaydet(cikti)
...
def test_eksik_dosya_manifesti_durdurur(tmp_path):
    motor = SYKMuhurTeslimOncesiManifesti(tmp_path)
