...
class SYKGoruntuKanitManifesti:
    SURUM = "1.0"

    KAYNAKLAR = (
        "src/syk_core/goruntu/canli_analiz_paneli.py",
        "src/syk_core/goruntu/goruntu_kanit_baglanti.py",
        "src/syk_core/goruntu/goruntu_uzmani_modeli.py",
    )

    TESTLER = (
        "tests/test_canli_analiz_paneli.py",
        "tests/test_goruntu_kanit_baglanti.py",
        "tests/test_goruntu_uzmani_modeli.py",
    )
        ...
        for goreli in self.tum_dosyalar:
            ...
            if not tam_yol.is_file():
                raise FileNotFoundError(
                    f"Goruntu zinciri dosyasi bulunamadi: {goreli}"
                )
